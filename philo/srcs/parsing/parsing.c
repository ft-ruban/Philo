...
static int	convert_argv_to_struct_utils(char *argv[], t_settings *set)
{
	...
	set->t_die = (ft_atol(argv[2]) * 1000); // * 1000 pour rendre directement utilisable par usleep.
	if (set->t_die < 0 || strlen(argv[2]) > 16)
		return (INVALID_DIE);
	set->t_eat = (ft_atol(argv[3])  * 1000);
	if (set->t_eat < 0 || strlen(argv[3]) > 16)
		return (INVALID_EAT);
	set->t_sleep = (ft_atol(argv[4]) * 1000);
	if (set->t_sleep < 0 || strlen(argv[4]) > 16)
...
static int	error_msg(t_settings *set, int return_value)
{
	...
	if (return_value == INVALID_DIE)
		write(2,
			"philo: INVALID_DIE: invalid input enter a value between 0 and 9223372036854775 ms\n",
			83);
	if (return_value == INVALID_EAT)
		write(2,
			"philo: INVALID_EAT: invalid input enter a value between 0 and 9223372036854775 ms\n",
			83);
	if (return_value == INVALID_SLEEP)
		write(2,
			"philo: INVALID_SLEEP: invalid input enter a value between 0 and 9223372036854775 ms\n",
			85);
	if (return_value == INVALID_MAXMEAL)
		write(2,
			"philo: INVALID_MAXMEAL: invalid input enter a value between 0 and 9223372036854775 ms\n",
			87);
	free(set);
	return (RETURN_ERROR);
}
